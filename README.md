...
The primary analysis encompasses the initial stages of data preparation, which are critical for ensuring the generation of high-quality datasets. This phase involves key processes such as trimming reads and checking for adapter sequences. By thoroughly addressing these steps early on, we significantly enhance the quality and reliability of the downstream analyses.
...
- **Trimming Reads**
    - Trimmomatic is used to trim the raw FASTQ files to remove low-quality bases and adapter sequences, producing cleaned FASTQ files.
...
The secondary analysis involves the assembly of the genome, organizing the sequence data into a coherent structure. This is followed by the generation of variant calls, where genetic differences between the assembled genome and the reference are identified. These steps are crucial for detecting and analyzing genetic variation.
...
**Warning**: Do not forget to change the working directory; otherwise, it may cause the pipeline to malfunction.
...
- **nsamples**: Number of individuals (samples) to be processed in each run.
...
![vcf-file](https://github.com/user-attachments/assets/0d3900ec-76ce-45ee-b396-9c032135ae2e)
